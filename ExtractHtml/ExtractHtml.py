...
logging.basicConfig(filename='ExtractHtml.log',level=logging.DEBUG,format='%(asctime)s %(message)s' )
ts = time.time()
...
for index in range(len(divsSectionTitles)):
    logging.info("Section Titles: \t      %s \t", sectionslist[index])
...
for index in range(len(nameslist)):
    logging.info("Topic: \t      %s \t  %s ", nameslist[index], durationlist[index])
#print (*nameslist, sep='\n')
